...
def search():
    ...
    res_matches = Restaurant.query.filter(Restaurant.name.contains(query) | Restaurant.category.contains(query)).all()
    # Search in menu items and get their restaurants
    item_matches = MenuItem.query.filter(MenuItem.name.contains(query) | MenuItem.category.contains(query)).all()
...
def checkout():
    ...
    new_order = Order(user_id=current_user.id, total_price=total, items=",".join(items_list))
    ...
    socketio.emit('order_status', {'order_id': new_order.id, 'status': 'Preparing'})
